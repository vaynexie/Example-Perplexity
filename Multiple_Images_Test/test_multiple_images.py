...
There are totally 1000 classes in ImageNet, for detailed classes, can check the file 'imagenet_class_index.json'
Note that, when input the labels into our code, we require the label is in the form of ImageNet class
...
#function for running the CNN models for the image 
def test(file_name, net_name_list, test_mode):
    global correct_prediction, counter
    ...
    for net_name in net_name_list:
        ...
        for single_path in record_paths:
            ...
            counter += 1
            print("current step: %d" % counter)
            print("prediction cost %f second" %
                  (time.time() - start_time))
#function for optimizing the running process
...
#function for computing the X-perplexity, C-perplexity, top 5 X-confusion classes, top 5 C-confusion classes for the images
def get_perplexity(file_name, gt, test_mode, how_many_top=5):
    ...
    print("top %d c confusion classes are" % how_many_top, top_5_c)
    if test_mode != 1:
        print("top %d x confusion classes are" % how_many_top, top_5_x)
...
#function for obtaining example images for the conufusion classes
...
#function for obtaining images with similar perplexity
